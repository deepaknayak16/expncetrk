...
class SbiParser : BankParser {
    companion object {
        ...
        // Pattern to extract merchant: "to [MERCHANT]" or "at [MERCHANT]" or "Info: [MERCHANT]"
    ...
    override fun parse(smsBody: String, timestamp: Long): ParsedSms? {
        ...
        val merchant = MERCHANT_REGEX.find(cleanBody)?.groupValues?.get(1)?.trim() ?: if (isDebit) "SBI Debit" else "SBI Credit"

        return ParsedSms(amount, type, merchant, "SBI", time)
    }
}
