...
class KotakParser : BaseBankParser("KOTAK") {
    ...
    // Removed "from" from keywords to avoid capturing the source account as merchant (Fix for BANK-XXXX format)
    override val merchantRegex = "(?i)(?:to|at|VPA[:/]|INFO\\*)\\s+([^\\s\\d][^\\.\\s]+(?:\\s+[^\\s\\d][^\\.\\s]+)*?)(?=\\s+\\bOn\\b\\s+\\d|\\s+\\bRef\\b|\\s+\\bRefNo\\b|\\.|$)".toRegex(RegexOption.IGNORE_CASE)
    override val secondaryMerchantRegex = "(?i)(?:to|at|VPA[:/])\\s+([^\\s\\d][^\\.\\s]+(?:\\s+[^\\s\\d][^\\.\\s]+)*?)(?=\\s+\\bOn\\b\\s+\\d|\\s+\\bRef\\b|\\s+\\bRefNo\\b|\\.|$)".toRegex(RegexOption.IGNORE_CASE)
}
